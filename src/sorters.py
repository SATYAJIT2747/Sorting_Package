# src/sorters.py
"""Contains implementations for all sorting algorithms."""
    ...
    def sort(self, data: List[int], ascending: bool = True) -> List[int]:
        ...
        while gap > 0:
            ...
            for i in range(gap, list_length):
                ...
                # add new_data[i] to the elements that have been gap sorted
                temp = new_data[i]
                
                # shift earlier gap-sorted elements up until the correct
                # location for new_data[i] is found
                j = i
                while j >= gap and \
                    ((ascending and new_data[j - gap] > temp) or \
                     (not ascending and new_data[j - gap] < temp)):
                    
                    new_data[j] = new_data[j - gap]
                    j -= gap
                    
                # put temp (the original new_data[i]) in its correct location
                new_data[j] = temp
